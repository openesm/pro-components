<script lang="ts">
import type { CSSProperties } from 'vue'
import type { DialogProps   } from 'element-plus'
import type { SdDialogTypes, SdDialogContext } from './types'

import { defineComponent, computed, ref, watch, reactive, toRef } from 'vue'
import { ElDialog, ElScrollbar, ElButton, ElIcon         } from 'element-plus'
import { Close as IconClose                              } from '@element-plus/icons-vue'
import { sdDialogProps, sdDialogEmits, sdDialogOmitKeys  } from './dialog'
import { isString, isFunction, omit                      } from 'lodash-es'
import { globalConfig } from '../../../config'

export default defineComponent({
    name      : 'SdDialog',
    components: { ElDialog, ElScrollbar, ElButton, ElIcon, IconClose },
    props     : sdDialogProps,
    emits     : sdDialogEmits,
    setup(props, ctx) {
        const $dialog: SdDialogContext = {
            scrollTo,
            setScrollTop,
            setScrollLeft,
            getScrollbar,
            setOption,
            getOption,
            show,
            hide,
            isVisible,
            isSubmitting,
        }

        const scroll_ref = ref() // 滚动容器引用
        const state = reactive<SdDialogTypes.WriteOptions>({
            visible              : props.modelValue,
            width                : props.width,
            height               : props.height,
            maxHeight            : props.maxHeight,
            showClose            : props.showClose,
            showFooter           : props.showFooter,
            fullscreen           : props.fullscreen,
            draggable            : props.draggable,
            title                : props.title,
            submitting           : props.submitting,
            showCancelButton     : props.showCancelButton,
            disabledCancelButton : props.disabledCancelButton,
            cancelButtonText     : props.cancelButtonText,
            showConfirmButton    : props.showConfirmButton,
            disabledConfirmButton: props.disabledConfirmButton,
            confirmButtonText    : props.confirmButtonText,
        })

        // 显示开关
        watch(() => props.modelValue, (val) => { state.visible = val })
        watch(() => state.visible   , (val) => { ctx.emit('update:modelValue', val) })

        watch(() => props.showClose , (val) => { state.showClose  = val }) // 关闭按钮显示开关
        watch(() => props.showFooter, (val) => { state.showFooter = val }) // 底部操作区域显示开关
        watch(() => props.fullscreen, (val) => { state.fullscreen = val }) // 全屏开关
        watch(() => props.draggable , (val) => { state.draggable  = val }) // 拖拽开关
        watch(() => props.title     , (val) => { state.title      = val }) // 弹窗标题
        watch(() => props.width     , (val) => { state.width      = val }) // 弹窗宽度
        watch(() => props.height    , (val) => { state.height     = val }) // 弹窗高度
        watch(() => props.maxHeight , (val) => { state.maxHeight  = val }) // 弹窗最大高度
        watch(() => props.submitting, (val) => { state.submitting = val }) // 提交中状态

        // 取消按钮
        watch(() => props.showCancelButton    , (val) => { state.showCancelButton     = val })
        watch(() => props.disabledCancelButton, (val) => { state.disabledCancelButton = val })
        watch(() => props.cancelButtonText    , (val) => { state.cancelButtonText     = val })

        // 确认按钮
        watch(() => props.showConfirmButton    , (val) => { state.showConfirmButton     = val })
        watch(() => props.disabledConfirmButton, (val) => { state.disabledConfirmButton = val })
        watch(() => props.confirmButtonText    , (val) => { state.confirmButtonText     = val })

        // el-dialog 组件属性配置
        const dialog_props$ = computed(() => omit(props, ...sdDialogOmitKeys) as DialogProps)

        // 内容区域滚动样式
        const scrollbar_style$ = computed(() => {
            const styles: CSSProperties = {}
            let height: string | number = ''
            if (state.fullscreen) {
                height = '100%'
            } else {
                height = isString(state.height) ? state.height : `${ state.height }px`
            }
            styles.height = height

            // 补充最大内容区域高度
            if (state.maxHeight !== undefined) {
                styles.maxHeight = isString(state.maxHeight) ? state.maxHeight : `${ state.maxHeight }px`
            }

            return styles
        })

        // --------------------------------------------------------------------------------------

        // 点击关闭按钮
        const onClickClose = () => {
            if (isFunction(props.beforeClose)) {
                props.beforeClose((cancel?: boolean) => {
                    state.visible = cancel ?? false
                })
            } else if (!props.submitting) {
                state.visible = false
            }
        }

        // 点击取消按钮
        const onClickCancel = () => {
            state.visible = false
            dispatchEvent('cancel')
        }

        // 点击确认按钮
        const onClickConfirm = () => {
            dispatchEvent('confirm')
        }

        // --------------------------------------------------------------------------------------
        // 是否可关闭抽屉
        // TODO: 待检查外部是否使用 before-close 属性，后续调整传入 $dialog 上下文
        const onBeforeClose = (done: (cancel?: boolean) => void) => {
            if (isFunction(props.beforeClose)) {
                props.beforeClose(done)
            } else {
                done(props.submitting)
            }
        }

        // 监听弹窗打开
        function onOpen() {
            globalConfig.event?.onModalShow('dialog', props)
            dispatchEvent('open')
        }

        // 监听弹窗关闭
        function onClose() {
            globalConfig.event?.onModalHide('dialog', props)
            dispatchEvent('close')
        }

        // --------------------------------------------------------------------------------------

        // 派发事件
        function dispatchEvent(name: string, params?: Record<string, any>) {
            ctx.emit(name as any, { ...params, $dialog })
        }

        // 滚动到一组特定坐标
        /** deprecated 后续统一使用 getScrollbar 处理 */
        function scrollTo(options: ScrollToOptions | number, yCoord?: number) {
            scroll_ref.value?.scrollTo(options, yCoord)
        }

        // 设置滚动条到顶部的距离
        /** deprecated 后续统一使用 getScrollbar 处理 */
        function setScrollTop(scrollTop: number) {
            scroll_ref.value?.setScrollTop(scrollTop)
        }

        // 设置滚动条到左边的距离
        /** deprecated 后续统一使用 getScrollbar 处理 */
        function setScrollLeft(scrollLeft: number) {
            scroll_ref.value?.setScrollLeft(scrollLeft)
        }

        // 取得滚动组件引用
        function getScrollbar() { return scroll_ref.value }

        // 显示弹窗
        function show() { state.visible = true  }

        // 隐藏弹窗
        function hide() { state.visible = false }

        // 是否正在显示中
        function isVisible() { return state.visible }

        // 是否正在提交中状态
        function isSubmitting() { return state.submitting }

        // 设置弹窗指定选项值
        function setOption(options: Partial<SdDialogTypes.WriteOptions>) {
            if (!options) return
            Object.assign(state, options)
        }

        // 取得弹窗选项指定值，不传返回全部
        function getOption(key?: string): any {
            if (!key) return { ...state }
            if (key in state) {
                return (state as any)[key]
            } else {
                return (props as any)[key]
            }
        }

        return {
            state,
            visible   : toRef(state, 'visible'),
            submitting: toRef(state, 'submitting'),
            dialog_props$,
            scrollbar_style$,
            onBeforeClose,
            onClickClose,
            onClickCancel,
            onClickConfirm,
            onOpen,
            onClose,
            scroll_ref,
            scrollTo,
            setScrollTop,
            setScrollLeft,
            getScrollbar,
            setOption,
            getOption,
            show,
            hide,
            isVisible,
            isSubmitting,
            dispatchEvent,
        }
    },
})
</script>

<template>
    <ElDialog
        v-bind="dialog_props$"
        v-model="state.visible"
        :fullscreen="state.fullscreen"
        :draggable="state.draggable"
        :width="state.width"
        :show-close="false"
        :before-close="onBeforeClose"
        :custom-class="`sd-dialog ${ customClass }`"
        @open="onOpen"
        @opened="dispatchEvent('opened')"
        @close="onClose"
        @closed="dispatchEvent('closed')"
    >
        <!-- 顶部区域 -->
        <template #header>
            <div class="sd-dialog__header" :style="headerStyle">
                <slot name="header">
                    <span class="sd-dialog__header-title" :style="titleStyle">
                        {{ state.title }}
                    </span>
                </slot>

                <slot name="header-extra" />

                <div v-if="state.showClose" class="sd-dialog__close-icon" @click="onClickClose">
                    <ElIcon :size="closeIconSize" :color="closeIconColor">
                        <IconClose />
                    </ElIcon>
                </div>
            </div>
        </template>

        <!-- 内容之前 -->
        <slot name="body-before" />

        <!-- 内容区域 -->
        <ElScrollbar
            v-if="state.fullscreen || state.height !== 'auto' || state.maxHeight !== undefined"
            ref="scroll_ref"
            v-bind="scrollbarProps ?? scrollbarConfig"
            :wrap-style="scrollbar_style$"
            @scroll="$emit('scroll', $event)"
        >
            <div class="sd-dialog__body" :style="bodyStyle">
                <slot />
            </div>
        </ElScrollbar>

        <div v-else class="sd-dialog__body" :style="bodyStyle">
            <slot />
        </div>

        <!-- 内容之后 -->
        <slot name="body-after" />

        <!-- 底部区域 -->
        <div v-if="state.showFooter" class="sd-dialog__footer" :style="footerStyle">
            <slot name="footer">
                <div v-if="$slots.extra" class="sd-dialog__footer-extra">
                    <slot name="extra" />
                </div>
                <ElButton
                    v-if="state.showCancelButton"
                    :disabled="state.disabledCancelButton || state.submitting"
                    @click="onClickCancel"
                >
                    {{ state.cancelButtonText }}
                </ElButton>
                <ElButton
                    v-if="state.showConfirmButton"
                    type="primary"
                    :loading="state.submitting"
                    :disabled="state.disabledConfirmButton || state.submitting"
                    @click="onClickConfirm"
                >
                    {{ state.confirmButtonText }}
                </ElButton>
            </slot>
        </div>
    </ElDialog>
</template>
